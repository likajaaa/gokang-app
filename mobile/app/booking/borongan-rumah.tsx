...
console.log('[Booking] borongan-rumah.tsx module loaded');

type BuildingType = 'apartment' | 'ruko' | 'rumah';
...
export default function BookingBoronganRumahScreen() {
  console.log('[Booking] BookingBoronganRumahScreen render');
  ...
  const openCamera = async () => {
    ...
    try {
      ...
      // quality 0.5 + exif false untuk reduce memory pressure (mencegah Android low-memory kill saat camera return)
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      console.log('[Booking] Converting', photos.length, 'photo(s) to base64...');
      ...
      console.log('[Booking] POST /booking/borongan/rumah', {
        ...payload,
        photos: `[${payload.photos.length} base64 images]`,
      });

      const response = await ordersApi.createBoronganRumah(payload);
      ...
      // Photo conversion errors dari imageHelpers — surface langsung tanpa override
      const errMessage: string = err?.message ?? '';
      if (errMessage.includes('Gagal mengkonversi foto')) {
        Alert.alert('Foto Bermasalah', errMessage);
        return;
      }
      ...
      if (status === 422 && errors) {
        // Show first validation error from each field
              ...
              return (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    backgroundColor: active ? '#E5EAF2' : 'white',
                    alignItems: 'center',
                  }}
                >
                  <Home size={22} color={active ? '#1E2D4F' : '#9CA3AF'} />
                  <Text
                    style={{
                      marginTop: 6,
                      fontFamily: 'Poppins_600SemiBold',
                      fontSize: 13,
                      color: active ? '#1E2D4F' : '#6B7280',
                    }}
                  >
                    {label}
                  </Text>
