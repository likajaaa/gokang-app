...
export default function BookingTukangHarianScreen() {
  ...
  const serviceName = params.service_name ?? 'Tukang';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log('[TukangHarian] POST /booking/tukang-harian', payload);
      const response = await ordersApi.createTukangHarian(payload);
      const data = response.data.data;

      Alert.alert(
        'Berhasil!',
        `Order ${data.order_code} berhasil dibuat.\nTotal: Rp ${data.total_amount.toLocaleString(
          'id-ID',
        )}`,
        [{ text: 'OK', onPress: () => router.back() }],
      );
    } catch (err: any) {
      console.error('[TukangHarian] Submit error:', err.response?.data || err.message);
    ...
    <View style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }}>
        ...
        >
          ...
          >
            Estimasi biaya akan dihitung berdasarkan durasi pekerjaan. Harga final dapat
            disesuaikan setelah tukang review pekerjaan.
          </Text>
        </View>
