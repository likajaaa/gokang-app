...
export default function InstallmentScreen() {
  ...
  const handleSelectInstallment = (item: Installment) => {
    router.push({
      pathname: '/payment/installment-detail',
      params: {
        orderId: orderId ?? '',
        orderCode: orderCode ?? '',
  ...
  const handleOpenTerms = (item: Installment) => {
    Alert.alert(
      `Syarat & Ketentuan ${item.name}`,
      `Pembayaran cicilan ${item.name} mengikuti syarat & ketentuan dari penyedia layanan.`,
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F3F4F6' }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        >
          {INSTALLMENT_LIST.map((item, index) => {
            const isLast = index === INSTALLMENT_LIST.length - 1;
            ...
            return (
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    Biaya Admin Rp. {item.adminFee.toLocaleString('id-ID')}
                  </Text>
