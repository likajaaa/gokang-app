...
export default function InternetBankingScreen() {
  ...
  const handleSelect = (item: InternetBankingOption) => {
    router.push({
      pathname: '/payment/internet-banking-detail',
      params: {
        orderId: orderId ?? '',
        orderCode: orderCode ?? '',
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F3F4F6' }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        >
          {INTERNET_BANKING_LIST.map((item, index) => {
            const isLast = index === INTERNET_BANKING_LIST.length - 1;
