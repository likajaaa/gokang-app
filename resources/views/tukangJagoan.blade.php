...
@section('content')
    ...
    <!-- section 1 -->
  <section class="relative w-full min-h-screen flex flex-col items-center justify-center text-center overflow-hidden bg-white">
  <!-- 1 Gambar background tunggal -->
  <img src="/img/cbk.png" alt="background" class="absolute inset-0 w-full h-full object-cover z-0">

  <!-- Konten utama -->
  <div class="relative z-10 flex flex-col items-center justify-center mt-10 md:mt-10 px-6">
    <h1 class="text-3xl md:text-5xl font-extrabold text-gray-800 mb-3 drop-shadow-lg">
      Lebih Jago Gabung <span class="text-red-600">GoKang</span>
    </h1>
    <p class="text-gray-700 text-base md:text-lg mb-80 drop-shadow-md">
      Ini saatnya profesi tukang ambil bagian di era digital
    </p>

    <!-- Teks bawah -->
    <div class="relative z-10 mt-16 max-w-xl">
      <h2 class="text-2xl md:text-4xl font-extrabold text-white mb-2 drop-shadow-lg">Perbaiki Rumah</h2>
      <p class="text-white text-base md:text-2xl drop-shadow-md">
        Lebih dari 5 juta unit rumah huni Jabodetabek perlu perbaikan dan perawatan tiap hari
      </p>
    </div>

    <!-- Panah navigasi -->
    <div class="flex space-x-4 mt-8">
      <button id="prevBtn" class="w-10 h-10 flex items-center justify-center rounded-full bg-white/90 shadow-lg hover:bg-gray-100 hover:scale-105 transition">
      ...
      <button id="nextBtn" class="w-10 h-10 flex items-center justify-center rounded-full bg-white/90 shadow-lg hover:bg-gray-100 hover:scale-105 transition">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-gray-800" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<script>
  // (Opsional) Navigasi antar slide jika ingin menambah gambar lain nanti
  const backgrounds = ["assets/bg-gokang.png", "assets/bg-gokang2.png", "assets/bg-gokang3.png"];
  let current = 0;
  const bgImg = document.querySelector("section img");

  document.getElementById("nextBtn").addEventListener("click", () => {
    current = (current + 1) % backgrounds.length;
    bgImg.src = backgrounds[current];
  });
    ...
    <section class="relative bg-white py-16 md:ml-[-26px] overflow-hidden px-6 md:px-10">
        <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 items-center gap-12">
            ...
            <div class="text-start lg:text-left">
                <h2
                    class="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white leading-relaxed md:leading-relaxed">
                    Satu Aplikasi,<br />
